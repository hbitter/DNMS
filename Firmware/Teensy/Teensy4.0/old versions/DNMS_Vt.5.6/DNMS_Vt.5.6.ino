 ...
 *                                                                      *
    ...
    and Ifineon microphone IM72D128, PDM output via ADAU7002ACBZ to I²S 
    input of Teensy4
...
#define SOFTWARE_VERSION "DNMS Version t.5.6"  // standalone testprogram for Teensy4 and IM72D128 micro based on FFT
/**********************************************************************************
 *                                                                                *
 *      Standalone testprogram                                                    *
 *      version for Teensy4 and IM72D128 microphone                               *
...
*/
#define CALC_PERIOD 125  // Time in ms for calculationg Leq values

#define DEFAULT_NUMBER_OF_CALC_PERIODS_FOR_TEST_OUTPUT 24            // default number of 125ms calculations periods to output data
...
uint16_t number_of_calc_periods;
uint16_t counter_of_calc_periods_for_output;
char s1[8];
...
#define window 0.3750  // Hanning
  ...
  number_of_calc_periods = DEFAULT_NUMBER_OF_CALC_PERIODS_FOR_TEST_OUTPUT;



  Serial.println(" "); Serial.println("DNMS Teensy4.0 standalone test program");
  Serial.println(" "); Serial.print("Version: "); Serial.print(SOFTWARE_VERSION); Serial.println("  for IM72D128 microphone");
  Serial.println(" "); Serial.println("output of all data every: 24 x 125ms - the default output period");
  Serial.println("you can change the output period by inputting now a different multiplication factor of the 125ms measurement interval");
  Serial.println("if you do no input within the next 5 seconds the default factor 24 will apply, which gives an output period of 3 seconds");
  Serial.println(" ");
  Serial.println("the output of values from left to right is: LZeq, LZmin, LZmax, followed by the 31 1/3 octave values from 20Hz up to 20.000Hz");
  Serial.println("the LA-values are given in the next line");
  Serial.println("if the output period is less than 500ms, the 1/3 octave values are not output");
  Serial.println(" ");
 
  Serial.setTimeout(5000);
  long eingabe = Serial.parseInt();
  if (eingabe == 0) {
    eingabe = number_of_calc_periods;
  }
  if (eingabe < 0) {
    eingabe = 1;
  }

  number_of_calc_periods = eingabe;
  counter_of_calc_periods_for_output = eingabe;
  Serial.print("time period multiplication factor is:"); Serial.println(eingabe);
  Serial.print("therefore output period is:"); Serial.print(eingabe*125); Serial.println(" ms");
...
void loop() {
  ...
  if (start_phase) {
    ...
    /**** CALC_PERIOD (standard 125ms) over? ***/
    if (currentMillis - startMillis >= CALC_PERIOD) {
      startMillis = currentMillis;
      // blinking of internal led ( 2 x CALC_PERIOD on and 2 x CALC_PERIOD off)
      led_count++;
      if (led_count == 2) {
        led_count = 0;
        if (led_on == false) {
      ...
      if (Serial.available() > 0) {  // # + Return input to start from beginning??
        if(Serial.read() == 35) {  // if yes let's do a restart
          SCB_AIRCR = 0x05FA0004;
        }
      }
      counter_of_calc_periods_for_output--; // decrement counter for output of data on serial USB port
...
//        Serial.print("z: "); Serial.print(z, 12); Serial.print("  leq_fft_A: "); Serial.println(leq_fft_A, 12);   
    ...
    } /*** End of CALC_PERIOD time loop (125ms) ***/

//    if (calculate_leq) {
      if (counter_of_calc_periods_for_output == 0) {  // do last calculation of data and than output data to serial USB port
      counter_of_calc_periods_for_output = number_of_calc_periods;
      ...
      if(number_of_calc_periods >= 4) {  // Output of 1/3 Octaves only if output period is >= 500 ms
        Serial.print(",");
        for (i = 1; i < 14; i++) {
      ...
      Serial.print(s1);      
       ...
       if(number_of_calc_periods >= 4) {  // Output of 1/3 Octaves only if output period is >= 1 second     
        Serial.print(",");
        for (i = 1; i < 14; i++) {
