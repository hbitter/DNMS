 ...
 *                                                                      *
    ...
    I2S Input from IvenSense Microphone ICS-43434, or Infineon IM72D128
 ...
 *                                                                      *
      DNMS - Digital Noise Measurement Sensor
      Copyright (C) 2021, 2022, 2023, 2024, 2025, 2026  Helmut Bitter
      Copyright (C) 2021, 2022, 2025, 2026  Burkhard Volkemer for FFT software parts
 *                                                                      *
...
#include "./Micro_corr_values_IM72D128_with_DLR_case.h"
#include "./Micro_no_corr_values.h"



/* the SOFTWARE-VERSION string must have an even sizeof !! */
#define SOFTWARE_VERSION "DNMS Version 5.9.x "  // Version DNMS Version 5.9.x for Teensy4 and ICS-43434 or IM72D128 microphone based on FFT
/**********************************************************************************
 *                                                                                *
 ...
 *      interval times:                                                           *
 *      1st interval for LAeq, LZeq, LCeq, LAmin, LZmin, LCmin as well as         * 
 *         LAmax, LZmax, LCmax and the 1/3 octaves of LA, LZ and LC               *
 *      2nd interval for LAeq, LZeq, LCeq, LAmin, LZmin, LCmin as well as         * 
 *         LAmax, LZmax, LCmax and the 1/3 octaves of LA, LZ and LC               *                                                      *
 *                                                                                *
 *                                                                                *
 *                                                                                *
...
Memory Usage on Teensy 4.0:
  FLASH: code:83376, data:164808, headers:8836   free for files:1774596
   RAM1: variables:359072, code:81192, padding:17112   free for local variables:66912
...
#define FFT_CALCULATIONS_IN_START_PHASE 44  // define number of first FFT calculations to throw away
...
#define micro_constant_ICS_43434 123.0102999565
#define micro_constant_IM72D128 133.0102999565
...
#define pin_fft4096 1
#define pin_fft1024 3
#define pin_I2C_1st 5
#define pin_I2C_2nd 7
...
/*
bool enable_2nd = true;
bool enable_LA = true;
bool enable_LZ = true;
bool enable_LA_2nd = true;
bool enable_LZ_2nd = true;
bool enable_LZ - spec = true;
bool enable_LA - spec = true;
bool enable_LZ - spec_2nd = true;
bool enable LA - spec_2nd = true;
bool enable_LC = false;
bool enable_LC_2nd = false;
bool enable_LC_spec = false;
bool enable_LC_spec_2nd = false;
*/
...
uint32_t mean_count_C = 0;
double leq_fft_C;
float leq_C_max = 0;
float leq_C_min = 0;
double leq_g_C = 0;
double sum_leq_g_C = 0;
double delog_leq_C;
float last_leq_g_C;
float last_leq_C_min;
float last_leq_C_max;
...
uint32_t mean_count_C_2nd = 0;
double leq_fft_C_2nd;
float leq_C_max_2nd = 0;
float leq_C_min_2nd = 0;
double leq_g_C_2nd = 0;
double sum_leq_g_C_2nd = 0;
float last_leq_g_C_2nd;
float last_leq_C_min_2nd;
float last_leq_C_max_2nd;
...
double freq1024_C[512];

float fft_all_A;
float mean_fft_all_A = 0;
float fft_all_C;
float mean_fft_all_C = 0;
float fft_all_Z;
...
double freq_band_C[32];
double mean_freq_band_C[32] = { 0 };
double leq_freq_band_C[32];
double sum_leq_freq_band_C[32] = { 0 };
double leq_freq_band_g_C[32] = { 0 };
double last_leq_freq_band_g_C[32] = { 0 };
...
double mean_freq_band_C_2nd[32] = { 0 };
double leq_freq_band_C_2nd[32];
double sum_leq_freq_band_C_2nd[32] = { 0 };
double leq_freq_band_g_C_2nd[32] = { 0 };
double last_leq_freq_band_g_C_2nd[32] = { 0 };
...
double freq_band1024_C[18];
double mean_freq_band1024_C[18] = { 0 };
double leq_freq_band1024_C[18];
double sum_leq_freq_band1024_C[18] = { 0 };
double leq_freq_band1024_g_C[18] = { 0 };
...
double mean_freq_band1024_C_2nd[18] = { 0 };
double leq_freq_band1024_C_2nd[18];
double sum_leq_freq_band1024_C_2nd[18] = { 0 };
double leq_freq_band1024_g_C_2nd[18] = { 0 };
...
const double *corr_A;
const double *corr_C;
const double *corr1024_Z;
const double *corr1024_A;
const double *corr1024_C;

uint16_t micro_type;  // 1 = ICS-43434, 2 = IM72D128
uint16_t micro;       // 1 = ICS-43434, 2 = IM72D128, 3 = IM72D128 with DLR casing, 4 = ICS-43434 no Correction, 5 = IM72D128 no correction
uint32_t blink_period;
double micro_constant;
...
void sum_C(int x, int y, int z) {
  freq_band_C[x] = 0.0f;
  do {
    freq_band_C[x] += freq_C[y++];
  } while (y <= z);
}
...
void sum1024_C(int x, int y, int z) {
  freq_band1024_C[x] = 0.0f;
  do {
    freq_band1024_C[x] += freq1024_C[y++];
  } while (y <= z);
}
...
// set output pins
  pinMode(led, OUTPUT);
  pinMode(pin_fft4096, OUTPUT);     // output pin for 1st measurement
  pinMode(pin_fft1024, OUTPUT);      // output pin for 2nd measurement
  pinMode(pin_I2C_1st, OUTPUT);     // I²C request 1st
  pinMode(pin_I2C_2nd, OUTPUT);     // I²C request 2nd
  ...
  // set default pointer for values for frequency correction
  ...
  micro_constant = micro_constant_ICS_43434;

  // set pointer to A-weighting values
  corr_A = A_weighting4096;
  corr1024_A = A_weighting1024;

  // set pointer to C-weighting values
...
void loop() {
  ...
  if (start_phase) {  // start phase for a number of
    ...
    if (dnms_fft.available()) {
      ...
      for (i = 14; i < 32; i++) {
        ...
        mean_freq_band_C_2nd[i] += freq_band_C[i];
      }
      #ifdef Teensy_debug
      digitalWrite(pin_fft4096, LOW);
      #endif
    ...
    if (dnms_fft1024.available()) {
      ...
      for (i = 0; i < 512; i++) {
        ...
        freq1024_C[i] = freq1024_Z[i] * corr1024_C[i];
      }
      ...
      sum1024_C(8, 66, 82);
      sum1024_C(9, 83, 104);
      sum1024_C(10, 105, 131);
      sum1024_C(11, 132, 166);
      sum1024_C(12, 167, 209);
      sum1024_C(13, 210, 263);
      // up to here used for 1/3 octave values
      ...
      for (i = 1; i < 14; i++) {  // only for the used ones
        ...
        mean_freq_band1024_C_2nd[i] += freq_band1024_C[i];
      }
      mean_count_fft1024++;
      #ifdef Teensy_debug
      digitalWrite(pin_fft1024, LOW);
    ...
    if (mean_count_fft >= NUMBER_OF_FFTs) {
      ...
      // LAeq, LZeq and LCeq aus FFT berechnen
      z = mean_fft_all_A * window_norm4096 / mean_count_fft;
      if (z == 0) {
        z = 1.0e-12;
      }
      leq_fft_A = micro_constant + 10 * log10(z);

      z = mean_fft_all_Z * window_norm4096 / mean_count_fft;
      if (z == 0) {
        z = 1.0e-12;
      }
      leq_fft_Z = micro_constant + 10 * log10(z);

      z = mean_fft_all_C * window_norm4096 / mean_count_fft;
      if (z == 0) {
        z = 1.0e-12;
      }
      leq_fft_C = micro_constant + 10 * log10(z);
      ...
      // compare new LCeq to saved LCeq min and save, if it is smaller than old LCeq min
      if (leq_C_min == 0) {
        leq_C_min = leq_fft_C;
      } else {
        if (leq_fft_C < leq_C_min) {
          leq_C_min = leq_fft_C;
        }
      }

      // now for 2nd: compare new LCeq to saved LCeq min and save, if it is smaller than old LCeq min
      if (leq_C_min_2nd == 0) {
        leq_C_min_2nd = leq_fft_C;
      } else {
        if (leq_fft_C < leq_C_min_2nd) {
          leq_C_min_2nd = leq_fft_C;
        }
      }
      ...
      // compare new LCeq to saved LCeq max and save, if it is bigger than old LCeq max
      if (leq_C_max == 0) {
        leq_C_max = leq_fft_C;
      } else {
        if (leq_fft_C > leq_C_max) {
          leq_C_max = leq_fft_C;
        }
      }

      // now for 2nd: compare new LCeq to saved LCeq max and save, if it is bigger than old LCeq max
      if (leq_C_max_2nd == 0) {
        leq_C_max_2nd = leq_fft_C;
      } else {
        if (leq_fft_C > leq_C_max_2nd) {
          leq_C_max_2nd = leq_fft_C;
        }
      }
      ...
      // recalculate the overall LCeq
      delog_leq_C = pow(10, leq_fft_C / 10);
      sum_leq_g_C += delog_leq_C;
      leq_g_C = 10 * log10(sum_leq_g_C / measure_loop_count);  // neuer gesamt LCeq
      ...
      // recalculate the overall LCeq_2nd
      sum_leq_g_C_2nd += delog_leq_C;
      leq_g_C_2nd = 10 * log10(sum_leq_g_C_2nd / measure_loop_count_2nd);  // neuer gesamt LCeq_2nd
      ...
      for (i = 14; i < 32; i++) {
        ...
        leq_freq_band_Z[i] = micro_constant + 10 * log10(z);
        ...
        leq_freq_band_A[i] = micro_constant + 10 * log10(z);
        ...
        z = mean_freq_band_C[i] * window_norm4096 / mean_count_fft;
        if (z == 0) {
          z = 0.0000001;
        }
        leq_freq_band_C[i] = micro_constant + 10 * log10(z);
        f1 = pow(10, leq_freq_band_C[i] / 10);
        sum_leq_freq_band_C[i] += f1;  // gesamt LCeq weiterrechnen
        sum_leq_freq_band_C_2nd[i] += f1;
        leq_freq_band_g_C[i] = 10 * log10(sum_leq_freq_band_C[i] / measure_loop_count_fft);              // neuer gesamt LCeq
        leq_freq_band_g_C_2nd[i] = 10 * log10(sum_leq_freq_band_C_2nd[i] / measure_loop_count_fft_2nd);  // neuer gesamt LCeq_2nd
        mean_freq_band_C[i] = 0;
      }
      mean_count_fft = 0;
    }
    ...
    if (mean_count_fft1024 >= NUMBER_OF_FFTs) {
      ...
      for (i = 1; i < 14; i++) {
        ...
        leq_freq_band1024_Z[i] = micro_constant + 10 * log10(z);
        ...
        leq_freq_band1024_A[i] = micro_constant + 10 * log10(z);
        ...
        z = mean_freq_band1024_C[i] * window_norm1024 / mean_count_fft1024;
        if (z == 0) {
          z = 0.0000001;
        }
        leq_freq_band1024_C[i] = micro_constant + 10 * log10(z);

        f1 = pow(10, leq_freq_band1024_C[i] / 10);
        sum_leq_freq_band1024_C[i] += f1;  // gesamt Leq weiterrechnen
        sum_leq_freq_band1024_C_2nd[i] += f1;
        leq_freq_band1024_g_C[i] = 10 * log10(sum_leq_freq_band1024_C[i] / measure_loop_count_fft1024);              // neuer gesamt Leq
        leq_freq_band1024_g_C_2nd[i] = 10 * log10(sum_leq_freq_band1024_C_2nd[i] / measure_loop_count_fft1024_2nd);  // neuer gesamt Leq
        mean_freq_band1024_C[i] = 0;
      }
    ...
    if (calculate_leq) {
      ...
      last_leq_g_C = leq_g_C;
      last_leq_C_min = leq_C_min;
      last_leq_C_max = leq_C_max;
      leq_g_C = 0;
      sum_leq_g_C = 0;
      leq_C_min = 0;
      leq_C_max = 0;
      ...
      for (i = 14; i < 32; i++) {
        ...
        last_leq_freq_band_g_C[i] = leq_freq_band_g_C[i];
        sum_leq_freq_band_C[i] = 0;
        leq_freq_band_g_C[i] = 0;
      }
      ...
      for (i = 1; i < 14; i++) {
        ...
        last_leq_freq_band_g_C[i] = leq_freq_band1024_g_C[i];
        sum_leq_freq_band1024_C[i] = 0;
        leq_freq_band1024_g_C[i] = 0;
      }
      calculate_leq = false;
      data_ready = 1;
    }
    ...
    if (calculate_leq_2nd) {
      ...
      last_leq_g_C_2nd = leq_g_C_2nd;
      last_leq_C_min_2nd = leq_C_min_2nd;
      last_leq_C_max_2nd = leq_C_max_2nd;
      sum_leq_g_C_2nd = 0;
      leq_g_C_2nd = 0;
      leq_C_min_2nd = 0;
      leq_C_max_2nd = 0;      
      ...
      for (i = 14; i < 32; i++) {
        ...
        last_leq_freq_band_g_C_2nd[i] = leq_freq_band_g_C_2nd[i];
        sum_leq_freq_band_C_2nd[i] = 0;
        leq_freq_band_g_C_2nd[i] = 0;        
      }
      ...
      for (i = 1; i < 14; i++) {
        ...
        last_leq_freq_band_g_C_2nd[i] = leq_freq_band1024_g_C_2nd[i];
        sum_leq_freq_band1024_C_2nd[i] = 0;
        leq_freq_band1024_g_C_2nd[i] = 0;        
      }
...
void i2c_request_from_master() {
  ...
  switch (i2c_state) {
    ...
    case 33:  // Read C 1/3 octav values part 1
      #ifdef Teensy_debug
      digitalWrite(pin_I2C_1st, HIGH);
      #endif    
      fft_C_values_i2c_write(1, 8);
      break;

    case 34:  // Read C 1/3 octav values part 2
      fft_C_values_i2c_write(9, 16);
      break;

    case 35:  // Read C 1/3 octav values part 3
      fft_C_values_i2c_write(17, 24);
      break;

    case 36:  // Read C 1/3 octav values part 4
      fft_C_values_i2c_write(25, 31);
      #ifdef Teensy_debug
      digitalWrite(pin_I2C_1st, LOW);
      #endif    
      break;

    case 37:  // Read LCeq 2nd
      idx = 0;
      #ifdef Teensy_debug
      digitalWrite(pin_I2C_2nd, HIGH);
      #endif
      f2b(last_leq_g_C_2nd, &b[0]);
      buf[idx++] = b[3];
      buf[idx++] = b[2];
      crc = dnms_common_generate_crc((uint8_t *)&buf[idx - 2], DNMS_WORD_SIZE);
      buf[idx++] = crc;
    ...
    case 38:  // Read C 1/3 octav 2nd values part 1
      #ifdef Teensy_debug
      digitalWrite(pin_I2C_2nd, HIGH);
      #endif    
      fft_C_values_2nd_i2c_write(1, 8);
      break;

    case 39:  // Read C 1/3 octav 2nd values part 2
      fft_C_values_2nd_i2c_write(9, 16);
      break;

    case 40:  // Read C 1/3 octav 2nd values part 3
      fft_Z_values_2nd_i2c_write(17, 24);
      break;

    case 41:  // Read C 1/3 octav 2nd values part 4
      fft_C_values_2nd_i2c_write(25, 31);
      #ifdef Teensy_debug
      digitalWrite(pin_I2C_2nd, LOW);
      #endif         
      break;
...
void fft_C_values_i2c_write(uint32_t start_index, uint32_t end_index) {
  ...
  idx = 0;
  for (i = start_index; i < end_index + 1; i++) {
    f2b(last_leq_freq_band_g_C[i], &b[0]);
    buf[idx++] = b[3];
    buf[idx++] = b[2];
    crc = dnms_common_generate_crc((uint8_t *)&buf[idx - 2], DNMS_WORD_SIZE);
    buf[idx++] = crc;
...
void fft_C_values_2nd_i2c_write(uint32_t start_index, uint32_t end_index) {
  ...
  idx = 0;
  for (i = start_index; i < end_index + 1; i++) {
    f2b(last_leq_freq_band_g_C_2nd[i], &b[0]);
    buf[idx++] = b[3];
    buf[idx++] = b[2];
    crc = dnms_common_generate_crc((uint8_t *)&buf[idx - 2], DNMS_WORD_SIZE);
    buf[idx++] = crc;
...
void i2c_receive_from_master(int num_bytes) {
  ...
  switch (command_received) {
    ...
    case 27:  // set DNMS to ICS-43434 microphone
      start_phase = true;
      AudioNoInterrupts();
      // command to set ICS-43434 microphone
      corr_Z = corr_Z_ICS43434;
      corr1024_Z = corr1024_Z_ICS43434;
      micro_type = 1;
      blink_period = BLINK_PERIOD_ICS43434;
    ...
    case 28:  // set DNMS to IM72D128 microphone
      start_phase = true;
      AudioNoInterrupts();
      // command to set IM72D128 microphone
      corr_Z = corr_Z_IM72D128;
      corr1024_Z = corr1024_Z_IM72D128;
      micro_type = 2;
      blink_period = BLINK_PERIOD_IM72D128;
      micro_constant = micro_constant_IM72D128;
      DNMS_reset();
      start_phase = false;
      break;

    case 29:  // set DNMS to IM72D128 with DLR case microphone
      start_phase = true;
      AudioNoInterrupts();
      // command to set IM72D128 microphone with DLR case
      corr_Z = corr_Z_IM72D128_DLR;
      corr1024_Z = corr1024_Z_IM72D128_DLR;
      micro_type = 2;
      blink_period = BLINK_PERIOD_IM72D128;
      micro_constant = micro_constant_IM72D128;
      DNMS_reset();
    ...
    case 30:  // set DNMS to ICS-43434 microphone without frequency correction
      start_phase = true;
      AudioNoInterrupts();
      // command to set ICS-43434 microphone without frequency correction
      corr_Z = no_corr;
      corr1024_Z = no_corr;
      micro_type = 1;
      blink_period = BLINK_PERIOD_ICS43434;
    ...
    case 31:  // set DNMS to IM72D128 microphone without frequency correction
      start_phase = true;
      AudioNoInterrupts();
      // command to set IM72D128 microphone without frequency correction
      corr_Z = no_corr;
      corr1024_Z = no_corr;
      micro_type = 2;
      blink_period = BLINK_PERIOD_IM72D128;
    ...
    case 38:  // Read C 1/3 octav values 2nd part 1
      i2c_state = 38;
      break;

    case 39:  // Read C 1/3 octav values 2nd part 2
      i2c_state = 39;
      break;

    case 40:  // Read C 1/3 octav values 2nd part 3
      i2c_state = 25;
      break;

    case 41:  // Read C 1/3 octav values 2nd part 4
      i2c_state = 41;
      break;
