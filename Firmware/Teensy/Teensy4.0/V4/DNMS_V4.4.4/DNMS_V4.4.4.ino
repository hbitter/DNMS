 ...
 *                                                                      *
      DNMS - Digital Noise Measurement Sensor
      Copyright (C) 2021, 2022  Helmut Bitter
...
#define SOFTWARE_VERSION "DNMS Version 4.4.4"  // Version 4.4.4 for Teensy4 mit FFT
/**********************************************************************************
 *                                                                                                                           *
 *      special version that allows two different measurement                     *
 *      interval times.                                                                                             *
...
#define RMS_CALCULATIONS_IN_START_PHASE 200 // define number of first RMS calculations to throw away
...
float mean_freq_band_2nd[32] = {0};
float leq_freq_band_2nd[32];
float sum_leq_freq_band_2nd[32] = {0};
float leq_freq_band_g_A_2nd[32] = {0};
float last_leq_freq_band_g_A_2nd[32] = {0};
...
float mean_freq_band1024_2nd[18] = {0};
float leq_freq_band1024_2nd[18];
float sum_leq_freq_band1024_2nd[18] = {0};
float leq_freq_band1024_g_A_2nd[18] = {0};
float last_leq_freq_band1024_g_A_2nd[18] = {0};
...
void loop() 
{
    if (start_phase && aa_rms_A.available()) 
    {
      ...
      if (number_of_start_phase_rms_calculations >= RMS_CALCULATIONS_IN_START_PHASE) 
      {
          start_phase = false;
      }
    } 
    else /*** here starts the normal loop besides the start phase ***/
    {
        ...
        if (dnms_fft.available()) 
        {
            ...
            for (i = 14; i < 32; i++) 
            {
                ...
                mean_freq_band_2nd[i] += freq_band[i];        
            }
            mean_count_fft++;     
        }
        ...
        if (dnms_fft1024.available()) 
        {
            ...
            {                    // only for the used ones
                ...
                mean_freq_band1024_2nd[i] += freq_band1024[i];        
            }
            mean_count_fft1024++;    
        }
        ...
        if (mean_count_A >= NUMBER_OF_LOOPS) 
        {
            ...
            // now for 2nd:  recalculate the overall LAeq
            sum_leq_g_A_2nd += delog_leq_A;
            measure_loop_count_A_2nd++;
            leq_g_A_2nd = 10 * log10(sum_leq_g_A_2nd / measure_loop_count_A_2nd); // neuer gesamt Leq
            ...
            if (mean_count_fft > 1) 
            {
                ...
                for (i = 14; i < 32; i++) 
                {
                    ...
                    f1 = pow(10, leq_freq_band[i] / 10);
                    sum_leq_freq_band[i] += f1;  // gesamt Leq weiterrechnen
                    sum_leq_freq_band_2nd[i] += f1;
                    leq_freq_band_g_A[i] = 10 * log10(sum_leq_freq_band[i] / measure_loop_count_fft); // neuer gesamt Leq
                    leq_freq_band_g_A_2nd[i] = 10 * log10(sum_leq_freq_band_2nd[i] / measure_loop_count_fft_2nd); // neuer gesamt Leq
                    mean_freq_band[i] = 0;
                }
                mean_count_fft = 0;
            }
            ...
            if (mean_count_fft1024 > 1) 
            {
                ...
                for (i = 1; i < 14; i++) 
                {
                    ...
                    f1 = pow(10, leq_freq_band1024[i] / 10);
                    sum_leq_freq_band1024[i] += f1;  // gesamt Leq weiterrechnen
                    sum_leq_freq_band1024_2nd[i] += f1;
                    leq_freq_band1024_g_A[i] = 10 * log10(sum_leq_freq_band1024[i] / measure_loop_count_fft1024); // neuer gesamt Leq
                    leq_freq_band1024_g_A_2nd[i] = 10 * log10(sum_leq_freq_band1024_2nd[i] / measure_loop_count_fft1024_2nd); // neuer gesamt Leq
