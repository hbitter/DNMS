 ...
 * Compiled with FS 4MB (FS: 1MB  OTA: ~1019KB)

. Variables and constants in RAM (global, static), used 42636 / 80192 bytes (53%)
║   SEGMENT  BYTES    DESCRIPTION
╠══ DATA     2736     initialized variables
╠══ RODATA   9492     constants       
╚══ BSS      30408    zeroed variables
. Instruction RAM (IRAM_ATTR, ICACHE_RAM_ATTR), used 63621 / 65536 bytes (97%)
║   SEGMENT  BYTES    DESCRIPTION
╠══ ICACHE   32768    reserved space for flash instruction cache
╚══ IRAM     30853    code in IRAM    
. Code in flash (default, ICACHE_FLASH_ATTR), used 646248 / 1048576 bytes (61%)
║   SEGMENT  BYTES    DESCRIPTION
╚══ IROM     646248   code in flash 
...
// increment on change
#define SOFTWARE_VERSION_STR "AIRROHR-DNMS-5.17"
...
unsigned time_for_wifi_config = 300000;
...
unsigned dnms_micro_type = 1;
bool gps_read = GPS_READ;
char temp_correction[LEN_TEMP_CORRECTION] = TEMP_CORRECTION;
bool dnms_read_din01 = DNMS_READ_DIN01;
...
*/
  ...
  page_content += FPSTR(WEB_BR);
  page_content += FPSTR(INTL_DNMS_MICROPHONE2);
  page_content += FPSTR(WEB_BR);

  page_content += FPSTR(TABLE_TAG_OPEN);
  add_form_input(page_content, Config_dnms_micro_type, FPSTR(INTL_DNMS_MICRO_TYPE), 5);
  page_content += FPSTR(TABLE_TAG_CLOSE_BR);


  page_content += FPSTR(WEB_BR_BR);
  page_content += FPSTR(TABLE_TAG_CLOSE_BR);
  add_form_checkbox_sensor(Config_dnms_read_din01, FPSTR(INTL_DNMS_READ_DIN01));
  page_content += FPSTR(TABLE_TAG_OPEN);
  add_form_input(page_content, Config_dnms_din01_pin, FPSTR(INTL_DNMS_DIN01_PIN), 5);
  page_content += FPSTR(TABLE_TAG_CLOSE_BR);
...
static void create_influxdb_string_from_data(String &data_4_influxdb, const String &data, const String &measure_name) {
  ...
  if (!err) {
    ...
    // should DIN01 be transmitted to InfluxDB/MQTT
    if (cfg::dnms_read_din01) {
      data_4_influxdb += ",DIN01=" + String(din01_state);
      din01_state = 0;
    }
      ...
      case 6:
        ...
        if (dnms_version[15] >= '3') {
          switch (cfg::dnms_micro_type) {
            ...
            case 3:  // IM72D128 with DLR case
              if (((dnms_version[13] == '5') && (dnms_version[15] >= '7')) || (dnms_version[13] >= '6')) {
                if (dnms_set_IM72D128_DLR_case() != 0) {
                  debug_outln_error(F("Error setting IM72D128 microphone with DLR case!"));
                  debug_outln_error(F("Check DNMS wiring"));
                  dnms_init_failed = true;
                  DNMS_error_count++;
                  deactivate_DNMS();
                } else {
                  debug_outln_info(F("IM72D128 microphone with DLR case set: OK"));
                }
              } else {
                debug_outln_error(F("Error setting microphone type - this type is not supported by the DNMS Teensy Version!"));
                debug_outln_error(F("Check configuration - DNMS set to hold!"));
                dnms_init_failed = true;
                deactivate_DNMS();
              }
              break;

            case 4:  // ICS-43434 without frequency respponse correction
              if (((dnms_version[13] == '5') && (dnms_version[15] >= '7')) || (dnms_version[13] >= '6')) {
                if (dnms_set_ICS43434_no_correction() != 0) {
                  debug_outln_error(F("Error setting ICS-43434 microphone without frequency response correction!"));
                  debug_outln_error(F("Check DNMS wiring"));
                  dnms_init_failed = true;
                  DNMS_error_count++;
                  deactivate_DNMS();
                } else {
                  debug_outln_info(F("ICS-43434 microphone without frequency response correction set: OK"));
                }
              } else {
                debug_outln_error(F("Error setting microphone type - this type is not supported by the DNMS Teensy Version!"));
                debug_outln_error(F("Check configuration - DNMS set to hold!"));
            ...
            case 5:  // IM72D128 without frequency respopnse correction
              if (((dnms_version[13] == '5') && (dnms_version[15] >= '7')) || (dnms_version[13] >= '6')) {
                if (dnms_set_IM72D128_no_correction() != 0) {
                  debug_outln_error(F("Error setting IM72D128 microphone without frequency response correction!"));
                  debug_outln_error(F("Check DNMS wiring"));
                  dnms_init_failed = true;
                  DNMS_error_count++;
                } else {
                  debug_outln_info(F("IM72D128 microphone without frequency response correction set: OK"));
                }
              } else {
                debug_outln_error(F("Error setting microphone type - this type is not supported by the DNMS Teensy Version!"));
                debug_outln_error(F("Check configuration - DNMS set to hold!"));
            ...
            default:  // default ICS-43434
              debug_outln_info(F("DNMS unkown firmware version!"));
              debug_outln_error(F("Check configuration - DNMS set to hold!"));
              dnms_init_failed = true;
              DNMS_error_count++;
              deactivate_DNMS();
...
//#define SERIAL_BUFFER_SIZE 256
  ...
  if (cfg::dnms_read_din01) {
    pinMode(cfg::dnms_din01_pin, INPUT);  // set GPIO Pin to input
  }
...
void loop(void) {
  ...
  if (msSince(led_timer) > led_status_period_1) {
    ...
    // should DIN01 be sampled?
    if (cfg::dnms_read_din01 && (din01_state == 0)) {
      din01_state = digitalRead(cfg::dnms_din01_pin);
    }
  }
