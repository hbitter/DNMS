...
int16_t dnms_set_ICS43434() {
  return dnms_i2c_write_cmd(DNMS_I2C_ADDRESS, DNMS_CMD_SET_ICS43434);
}

int16_t dnms_set_IM72D128() {
  return dnms_i2c_write_cmd(DNMS_I2C_ADDRESS, DNMS_CMD_SET_IM72D128);
}

int16_t dnms_set_IM72D128_DLR_case() {
  return dnms_i2c_write_cmd(DNMS_I2C_ADDRESS, DNMS_CMD_SET_IM72D128_DLR_case);
}

int16_t dnms_set_ICS43434_no_correction() {
  return dnms_i2c_write_cmd(DNMS_I2C_ADDRESS, DNMS_CMD_SET_ICS43434_no_correction);
}

int16_t dnms_set_IM72D128_no_correction() {
  return dnms_i2c_write_cmd(DNMS_I2C_ADDRESS, DNMS_CMD_SET_IM72D128_no_correction);
}
...
int16_t dnms_read_leq(uint32_t dnms_command, struct dnms_measurements *leq) {
  ...
  ret = dnms_i2c_read_cmd(DNMS_I2C_ADDRESS, dnms_command, data->uu, DNMS_NUM_WORDS(data));
  if (ret != STATUS_OK)
    return ret;
...
int16_t dnms_read_freq_spec(uint32_t dnms_command, struct dnms_measurements *leq) {
  ...
  for (i = 0; i < 4; i++) {
    if (i != 3) {
      ret = dnms_i2c_read_cmd(DNMS_I2C_ADDRESS, dnms_command + i, data->uu, DNMS_NUM_WORDS(data));
      stop_idx = 8;
    } else {
      ret = dnms_i2c_read_cmd(DNMS_I2C_ADDRESS, dnms_command + i, data->uu, DNMS_NUM_WORDS(data) - DNMS_WORD_SIZE);
      stop_idx = 7;
    }
    if (ret != STATUS_OK) {
      return ret;
    ...
    for (idx = 0; idx < stop_idx; idx++) {
      val.u = be32_to_cpu(data[idx].u);
      leq->leq_freq_spec_x[(i * 8) + idx] = val.f;
    }
  }
  return STATUS_OK;
}
