...
// increment on change
#define SOFTWARE_VERSION_STR "AIRROHR-DNMS-5.8"
...
namespace cfg {
  ...
  char db_name_influx[LEN_DB_NAME_INFLUX];
  ...
  void initNonTrivials(const char *id) {
    ...
    strcpy_P(db_name_influx, DB_NAME_INFLUX);
...
*/
  ...
  add_form_input(page_content, Config_db_name_influx, FPSTR(INTL_DB), LEN_DB_NAME_INFLUX - 1);
  page_content += FPSTR(WEB_BR_LF_B);
  page_content += FPSTR(INTL_MQTT_MEASUREMENT_TEXT);
  page_content += FPSTR(WEB_B_BR);
...
static unsigned long sendDataToOptionalApis(const String &data) {
  ...
  if (send_2nd_dnms) {
    dnms_now = dnms_2nd_now;
      ...
      sum_send_time += sendData(LoggerMQTT, data_4_influxdb, 0, cfg::host_mqtt, cfg::db_name_influx);
    }  
    if (cfg::send2influx) {
