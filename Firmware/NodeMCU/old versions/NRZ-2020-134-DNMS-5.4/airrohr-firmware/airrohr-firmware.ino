...
. Variables and constants in RAM (global, static), used 42232 / 80192 bytes (52%)
║   SEGMENT  BYTES    DESCRIPTION
╠══ DATA     2728     initialized variables
╠══ RODATA   9232     constants       
╚══ BSS      30272    zeroed variables

. Instruction RAM (IRAM_ATTR, ICACHE_RAM_ATTR), used 47789 / 65536 bytes (72%)
║   SEGMENT  BYTES    DESCRIPTION
╠══ ICACHE   16384    reserved space for flash instruction cache
╚══ IRAM     31405    code in IRAM    
. Code in flash (default, ICACHE_FLASH_ATTR), used 634164 / 1048576 bytes (60%)
║   SEGMENT  BYTES    DESCRIPTION
╚══ IROM     634164   code in flash 
 *  
 ************************************************************************/
...
// increment on change
#define SOFTWARE_VERSION_STR "NRZ-2021-134-DNMS-5.4"
...
static unsigned long sendData(const LoggerEntry logger, const String &data, const int pin, const char *host, const char *url) {
  ...
  if (http.begin(*client, s_Host, loggerConfigs[logger].destport, s_url, !!loggerConfigs[logger].session)) {
    ...
    if (result >= HTTP_CODE_OK && result <= 299) {
      ...
      time_t now = time(nullptr);
      debug_out(FPSTR(INTL_TIME_UTC), DEBUG_ERROR); Debug.print(String(ctime(&now)) + '\r');
...
//      debug_outln_error(String(logger));
      switch (logger) {
        ...
        default: debug_outln("not specified: ", DEBUG_ERROR); debug_outln_error(String(logger)); break;
      }
      debug_out(F("Request failed with HTTP error: "), DEBUG_ERROR);
//      debug_outln_error(String(result));
      switch (result) {
        case -1: debug_outln("connection failed", DEBUG_ERROR); break;
        case -2: debug_outln("send header failed", DEBUG_ERROR); break;
        case -3: debug_outln("send payload failed", DEBUG_ERROR); break;
...
//    Debug.println(data_4_influxdb);
    ...
    debug_out(FPSTR(INTL_TIME_UTC), DEBUG_ERROR); Debug.print(String(ctime(&dnms_now)) + '\r');
...
static void fetchSensorSHT3x(String &s) {
  ...
  if (isnan(h) || isnan(t)) {
    ...
    last_value_SHT3X_T = t - float(readCorrectionOffset(cfg::temp_correction));
