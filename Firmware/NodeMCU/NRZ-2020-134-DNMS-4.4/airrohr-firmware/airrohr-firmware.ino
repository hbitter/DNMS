 ...
 *    Copyright (C) 2022-2023  Helmut Bitter                            *
 *                                                                      *
 ...
 ************************************************************************
 *
 * latest build using esp8266 BSP 3.1.1 , do not use a lower version!
 * DATA:    [====      ]  49% (used 39460 bytes from 81920 bytes)
 * PROGRAM: [======    ]  60% (used 629228 bytes from 1044464 bytes)
 *  
 ************************************************************************/
...
// increment on change
#define SOFTWARE_VERSION_STR "NRZ-2021-134-DNMS-4.4"
 ...
 ******************************************************************/
#define sc_sending_intervall_ms 145000
...
namespace cfg
{
  ...
  unsigned dnms_i2c_gpio_scl = 2;
  unsigned dnms_i2c_gpio_sda = 0;
...
//  add_form_checkbox(Config_powersave, FPSTR(INTL_POWERSAVE));
  ...
  page_content += FPSTR(TABLE_TAG_OPEN);
  add_form_input(page_content, Config_dnms_i2c_gpio_scl, FPSTR(INTL_DNMS_I2C_GPIO_SCL), 5);
  add_form_input(page_content, Config_dnms_i2c_gpio_sda, FPSTR(INTL_DNMS_I2C_GPIO_SDA), 5);
  page_content += FPSTR(TABLE_TAG_CLOSE_BR);
...
static void webserver_values()
{
	...
	if (cfg::dnms_read_spectrum_active && cfg::dnms_2nd_interval_active)
	{
		for (unsigned i = 0; i < 31; i++)
    		{
...
static void fetchSensorDNMS_FFT(String &s)
{
  ...
  if (dnms_read_freq_spec(&dnms_values) == 0)
  {
    for (unsigned i = 0; i < 31; i++) 
    {
      last_value_dnms_laeq_freq_spec[i]  = dnms_values.leq_freq_spec_a[i];
    }
  }
  else
  {
...
static void fetchSensorDNMS_2nd(String &s)
{
	...
	{
		struct dnms_measurements dnms_values;
		if (dnms_read_leq_2nd(&dnms_values) == 0)
		{
			float dnms_corr_value = readCorrectionOffset(cfg::dnms_correction);
      last_value_dnms_laeq_2nd = dnms_values.leq_a + dnms_corr_value;
      last_value_dnms_la_min_2nd = dnms_values.leq_a_min + dnms_corr_value;
      last_value_dnms_la_max_2nd = dnms_values.leq_a_max + dnms_corr_value;      
	...
	if (dnms_error)
	{
		DNMS_error_count++;
    dnms_reset(); // try to reset dnms
		debug_outln_error(F("DNMS read 2nd failed"));
	}
	else
	{
...
static void fetchSensorDNMS_FFT_2nd(String &s)
{
  ...
  // ***** read dnms frequency spectrum if dnms firmware version = 3 and if configured
  if (dnms_read_freq_spec_2nd(&dnms_values) == 0)
  {
    for (unsigned i = 0; i < 31; i++) 
    {
      last_value_dnms_laeq_freq_spec_2nd[i]  = dnms_values.leq_freq_spec_a[i];
    }
  }
  else
  {
  ...
  {
    dnms_reset(); // try to reset dnms
    debug_outln_error(F("DNMS FFT 2nd read failed"));
  }
  else
  {
    // ***** read dnms frequency spectrum if dnms firmware version = 3 and if configured
...
{  
  if (!scd30.begin())
    ...
    Debug.print("SCD30 firmware: "); Debug.println(last_value_SCD30_firmware);  
    ...
    Debug.print("SCD30 measurement interval: "); Debug.println(scd30.getMeasurementInterval());        
    scd30.setAltitudeCompensation(uint16_t(readCorrectionOffset(cfg::height_above_sealevel)));
    Debug.print("SCD30 sensor altitude: "); Debug.println(scd30.getAltitudeCompensation()); 
    scd30.setTemperatureOffset(float(readCorrectionOffset(cfg::temp_correction)));
    Debug.print("SCD30 temperature offset: "); Debug.println(scd30.getTemperatureOffset()); 
...
static void initSCD4X()
{
   ...
   if (!scd4x.begin()) 
    ...
    Debug.print("SCD4X SerialNumber: "); Debug.println((char*)serialNumber); 
    ...
    Debug.print("SCD4X tmperature offset: "); Debug.println(scd4x.getTemperatureOffset());   
    scd4x.setSensorAltitude(uint16_t(readCorrectionOffset(cfg::height_above_sealevel)));
    Debug.print("SCD4X height above sealevel: "); Debug.println(scd4x.getSensorAltitude()); 
...
static void initSEN5X()
{
  ...
  sen5x.getTemperatureOffsetSimple(sen5x_temp);
  Debug.print("SEN5X tmperature offset: "); Debug.println(sen5x_temp); 
...
static void initDNMS()
{
  ...
  int int_version;
    ...
    if (int_version >= 3)  
		{
			// dnms spectrum is available
			debug_outln_info(F("DNMS frequency spectrum is available"));
      if (cfg::dnms_read_spectrum)
      {
        cfg::dnms_read_spectrum_active = true;
        debug_outln_info(F("and reading of DNMS spectrum is configured"));                
      } 
      else 
      {
        debug_outln_info(F("but reading of DNMS spectrum is not configured! Is this O.K.?"));        
      }
		} else {
			// dnms spectrum is not available but DNMS read spectrum is configured, so set dnms_read_spectrum_active to false
			debug_outln_info(F("DNMS frequency spectrum is not available"));
      			cfg::dnms_read_spectrum_active = false;
		}
      cfg::dnms_2nd_interval_active = false;
      if (int_version == 4)
      {
      // dnms 2nd interval is available in DNMS firmware
      debug_outln_info(F("DNMS 2nd interval is available"));
      if (cfg::dnms_2nd_interval)
      {
        cfg::dnms_2nd_interval_active = true;
        debug_outln_info(F("and 2nd interval is configured"));               
      }
      else
      {
        debug_outln_info(F("but 2nd interval is not configured! Is this O.K.?"));           
      }
    }
    else
    {
      // dnms 2nd interval is not available but DNMS 2nd interval is configured, so set dnms_2nd_interval_active to false
      debug_outln_info(F("DNMS 2nd interval is not available"));
      cfg::dnms_2nd_interval_active = false;      
    }
	}
}
...
static unsigned long sendDataToOptionalApis(const String &data)
{
  ...
  if(send_2nd_dnms)
  {
    ...
    if (cfg::send2influx)
    {
      ...
      RESERVE_STRING(data_4_influxdb, XXLARGE_STR);
...
void setup(void)
{
  ...
  Wire.begin(cfg::dnms_i2c_gpio_sda, cfg::dnms_i2c_gpio_scl);
  Wire.setClock(150000);
