 ...
 * DATA:    [====      ]  52% (used 41808 bytes from 80192 bytes)
 * PROGRAM: [======    ]  60% (used 632744 bytes from 1048576 bytes)
 *  
 ************************************************************************/
...
// increment on change
#define SOFTWARE_VERSION_STR "NRZ-2021-134-DNMS-5.2"
...
//    Debug.print("logger: "); Debug.print(logger); Debug.print(" result: "); Debug.println(result);

    if (result >= HTTP_CODE_OK && result <= 299) {
      debug_outln_info(F("Succeeded - "), s_Host);
      send_success = true;
    } else if (result >= HTTP_CODE_BAD_REQUEST || result < 0) {
      debug_out("http POST to: ", DEBUG_ERROR); debug_outln_error(String(logger));
      debug_out(F("Request failed with error: "), DEBUG_ERROR); debug_outln_error(String(result));
      debug_out(F("Details:"), DEBUG_ERROR); debug_outln_error(http.getString());
    }
    http.end();
  } else {
...
static void create_influxdb_string_from_data(String &data_4_influxdb, const String &data) {
  ...
  if (!err) {
    ...
    Debug.println("Fehler bei create_influxdb_string_from_data");
    debug_outln_error(FPSTR(DBG_TXT_DATA_READ_FAILED));
  }
}
