...
int16_t dnms_calculate_leq_2nd() {
  ...
  return dnms_i2c_write_cmd(DNMS_I2C_ADDRESS, DNMS_CMD_CALCULATE_LEQ_2nd);
}
...
int16_t dnms_read_data_ready_2nd(uint16_t *data_ready) {
  return dnms_i2c_read_cmd(DNMS_I2C_ADDRESS, DNMS_CMD_READ_DATA_READY_2nd, data_ready, DNMS_NUM_WORDS(*data_ready));
}
...
int16_t dnms_read_leq_2nd(struct dnms_measurements *leq) {
  int16_t ret;
  uint16_t idx;
  union {
    uint16_t uu[2];
  ...
  ret = dnms_i2c_read_cmd(DNMS_I2C_ADDRESS, DNMS_CMD_READ_LEQ_2nd, data->uu, DNMS_NUM_WORDS(data));
  if (ret != STATUS_OK)
    return ret;
...
int16_t dnms_read_freq_spec(struct dnms_measurements *leq) {
  int16_t ret;
  uint16_t idx;
  uint16_t stop_idx;
  uint16_t i;
  ...
  for (i = 0; i < 4; i++) {
    if (i != 3) {
      ret = dnms_i2c_read_cmd(DNMS_I2C_ADDRESS, DNMS_CMD_READ_FFT_PART1 + i, data->uu, DNMS_NUM_WORDS(data));
      stop_idx = 8;
    } else {
      ret = dnms_i2c_read_cmd(DNMS_I2C_ADDRESS, DNMS_CMD_READ_FFT_PART1 + i, data->uu, DNMS_NUM_WORDS(data) - DNMS_WORD_SIZE);
      stop_idx = 7;
    }
    if (ret != STATUS_OK) {
      return ret;
    ...
    if (i != 3) {
      ...
      DNMS_WORDS_TO_BYTES(data->uu, DNMS_NUM_WORDS(data) - DNMS_WORD_SIZE);
    }

    for (idx = 0; idx < stop_idx; idx++) {
      val.u = be32_to_cpu(data[idx].u);
      leq->leq_freq_spec_a[(i * 8) + idx] = val.f;
    }
  }
  return 0;
}
...
int16_t dnms_read_freq_spec_2nd(struct dnms_measurements *leq) {
  int16_t ret;
  uint16_t idx;
  uint16_t stop_idx;
  uint16_t i;
  ...
  for (i = 0; i < 4; i++) {
    if (i != 3) {
      ret = dnms_i2c_read_cmd(DNMS_I2C_ADDRESS, DNMS_CMD_READ_FFT_PART1_2nd + i, data->uu, DNMS_NUM_WORDS(data));
      stop_idx = 8;
    } else {
      ret = dnms_i2c_read_cmd(DNMS_I2C_ADDRESS, DNMS_CMD_READ_FFT_PART1_2nd + i, data->uu, DNMS_NUM_WORDS(data) - DNMS_WORD_SIZE);
      stop_idx = 7;
    }
    if (ret != STATUS_OK) {
      return ret;
