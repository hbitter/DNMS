...
int file_i2c_dnms;
int16_t number_dnms;
unsigned char i2c_buffer_dnms[64] = {0};
char *dev_name_dnms = (char*)"/dev/i2c-1";


int16_t open_i2c_dnms() {
    if ((file_i2c_dnms = open(dev_name_dnms, O_RDWR)) < 0) {
		//ERROR HANDLING: you can check errno to see what went wrong
        printf("Fehler bei Open i2c bus\n");
        printf("\n");
		return -1;
    }
	if (ioctl(file_i2c_dnms, I2C_SLAVE, DNMS_I2C_ADDRESS) < 0) {
        printf("Fehler bei ioctl DNMS nicht gefunden\n");
        printf("\n");
        return -2;    
    }
    return 0;   
...
int8_t dnms_i2c_read(uint8_t address, uint8_t* data, uint16_t count) {
    if (read(file_i2c_dnms, data, count) != count) {
        return -1;
    }
    return 0;
}
...
int8_t dnms_i2c_write(uint8_t address, const uint8_t* data, uint16_t count) {
	if (write(file_i2c_dnms, data, count) != count) {
        return -1;
    }
    return 0;
} 	    
